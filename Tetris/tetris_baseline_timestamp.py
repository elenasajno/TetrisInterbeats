...
def write_to_csv(text):
    filename = f"dati_tetris_{date_str}_subject{subject_number}_task{task_number}.csv"
    ...
    with open(filename, 'a', newline='') as file:
        ...
        writer.writerow([text,timestamp, tick, local_time])
...
def check(board,screen):
    ...
    effect = False
    rows = []
    lrows = 0  # Initialize lrows outside the if condition
    for i in range(20):
...
def game(screen,startinglevel):
    ...
    """
    print("inizio livello")
    write_to_csv("inizio livello")
    cleared = 0
    ...
    write_to_csv("starting level:"+ str(startinglevel))
...
def main():
    ...
    write_to_csv("start application")
    makeblockimages()
    pygame.init()
    size = (341,700)
...
if __name__=='__main__':
    ...
    t = input("Enter baseline lenght (in seconds): ")
    t = int(t)
    countdown(t)
